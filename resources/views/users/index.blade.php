<x-layouts.app title="Manajemen Pengguna" subtitle="Akun, role, lokasi, dan status akses">
    <div class="grid gap-6 xl:grid-cols-[1fr_22rem]">
        <section>
            <div class="table-wrap"><table class="data-table"><thead><tr><th>Pengguna</th><th>Role</th><th>Lokasi</th><th>Status</th><th class="text-right">Aksi</th></tr></thead><tbody>
        ...
        <aside class="card h-fit p-5 xl:sticky xl:top-24">
            ...
            <form id="user-form" method="POST" action="{{ route('users.store') }}" class="mt-5 space-y-4">@csrf <input id="user-method" type="hidden" name="_method" value="POST">
                ...
                <div><label class="label">Kata sandi</label><input id="user-password" type="password" name="password" class="field" minlength="8"><p class="mt-1 text-xs text-slate-500">Wajib untuk akun baru; kosongkan saat edit.</p></div>
                <div><label class="label">Role</label><select id="user-role" name="role" class="field">@foreach(config('rbac.roles') as $key=>$label)<option value="{{ $key }}">{{ $label }}</option>@endforeach</select></div>
