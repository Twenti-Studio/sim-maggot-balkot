...
<head>
    ...
    <title>{{ isset($title) ? $title.' · ' : '' }}SIM Rumah Maggot</title>
...
<body class="min-h-screen bg-stone-50">
    ...
    <aside data-sidebar class="fixed inset-y-0 left-0 z-40 flex w-72 -translate-x-full flex-col border-r border-stone-200 bg-white transition-transform lg:translate-x-0">
        <a href="{{ route('dashboard') }}" class="flex items-center gap-3 border-b border-stone-100 px-5 py-5">
            <img src="/icons/app-icon.png" alt="Logo SIM Rumah Maggot" class="size-11 rounded-xl object-contain">
            <div>
                <p class="font-bold leading-tight text-slate-900">SIM Rumah Maggot</p>
                <p class="mt-0.5 text-xs text-slate-500">Operasional terintegrasi</p>
            </div>
        </a>
        ...
        <nav class="flex-1 space-y-1 overflow-y-auto px-3 py-4" aria-label="Navigasi utama">
            ...
            @if(auth()->user()->hasPermission('report.export') && in_array(auth()->user()->role, ['super_admin', 'admin']))
                <a href="{{ route('exports.index') }}" class="nav-link {{ request()->routeIs('exports.*') ? 'nav-link-active' : '' }}">
                    <x-heroicon-o-arrow-down-tray class="size-5" /> Laporan & Export
                </a>
            @endif
            ...
            @if(auth()->user()->hasPermission('user.view') || auth()->user()->hasPermission('location.manage'))
                ...
                @if(auth()->user()->hasPermission('user.view'))
                    ...
                    <a href="{{ route('rbac.index') }}" class="nav-link {{ request()->routeIs('rbac.*') ? 'nav-link-active' : '' }}">
                        <x-heroicon-o-shield-check class="size-5" /> RBAC
                    </a>
                @endif
    ...
    <div class="min-h-screen lg:pl-72">
        <header class="sticky top-0 z-20 border-b border-stone-200 bg-white/95 backdrop-blur">
            <div class="flex h-16 items-center justify-between px-4 sm:px-6 lg:px-8">
                <div class="flex min-w-0 items-center gap-3">
                    ...
                    <div class="min-w-0">
                        <h1 class="truncate text-lg font-bold text-slate-900">{{ $title ?? 'SIM Rumah Maggot' }}</h1>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button data-enable-push class="grid size-11 place-items-center rounded-xl text-slate-600 hover:bg-stone-100" aria-label="Aktifkan push notification">
        ...
        <footer class="border-t border-stone-200 bg-white px-4 py-5 text-center text-xs text-slate-500 sm:px-6 lg:px-8">
            SIM Rumah Maggot · Dibangun dan dikelola oleh <a href="https://twenti.studio" target="_blank" rel="noopener" class="font-semibold text-brand-700 hover:text-brand-800">twenti.studio</a>
        </footer>
    </div>
</body>
</html>
